...
native_image_targets='image-base image-dns-blocky image-firewall image-tailnet-router image-bifrost image-network-probe'
native_scan_names='boetticher-base boetticher-dns-blocky boetticher-firewall boetticher-tailnet-router boetticher-bifrost boetticher-network-probe'
...
validate_remote_target() {
  ...
  for target in "$@"; do
    case "$runner:$target" in
	  build:image-base|build:image-dns-blocky|build:image-firewall|build:image-tailnet-router|build:image-bifrost|build:image-network-probe|build:images) ;;
	  scan:scan-base|scan:scan-dns-blocky|scan:scan-firewall|scan:scan-tailnet-router|scan:scan-bifrost|scan:scan-network-probe|scan:scan-images) ;;

...
main() {
  ...
  for required in "$payload/bin/boetticher" "$payload/bin/boetticher-status" "$payload/controller/bootstrap.yml" "$payload/controller/requirements.txt" "$payload/controller/ansible.cfg" "$payload/controller/roles/controller-baseline/files/boetticher-blinkt-driver"; do
  ...
  if [ -e "$release" ]; then
    ...
    chmod 0755 "$install_stage/bin/boetticher" "$install_stage/bin/boetticher-status" "$install_stage/controller/roles/controller-baseline/files/boetticher-blinkt-driver"

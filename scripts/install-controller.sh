...
main() {
  ...
  if [ -e "$release" ]; then
    ...
    chmod 0755 "$install_stage/bin/boetticher" "$install_stage/bin/boetticher-status" "$install_stage/bin/boetticher-lab-snapshot" "$install_stage/bin/boetticher-labviewer" "$install_stage/bin/arrstack" "$install_stage/controller/roles/controller-baseline/files/boetticher-blinkt-driver" "$install_stage/controller/proxmox/libexec/boetticher-build-openwrt-firewall" "$install_stage/controller/proxmox/libexec/boetticher-build-tailnet" "$install_stage/controller/proxmox/libexec/boetticher-build-arrstack-vm" "$install_stage/controller/proxmox/libexec/boetticher-firewall-test-host" "$install_stage/controller/proxmox/libexec/boetticher-host-speedtest" "$install_stage/controller/proxmox/libexec/boetticher-install-observability-providers" "$install_stage/controller/proxmox/libexec/boetticher-install-observability-collection" "$install_stage/controller/proxmox/libexec/boetticher-build-observability-base" "$install_stage/controller/observability/bin/gatus" "$install_stage/controller/observability/bin/bifrost" "$install_stage/controller/observability/bin/caddy"

...
from .render import blank_tile, screensaver_tile, tile
...
def render_screensaver(deck, frame: int) -> None:
    width, height = deck.key_image_format()["size"]
    image_format = deck.key_image_format().get("format", "JPEG")
    for index in range(deck.key_count()):
        _set_key_image(deck, index, screensaver_tile(width, height, index, frame, image_format))

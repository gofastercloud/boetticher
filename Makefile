...
image-check:
	...
	sh -n scripts/build-images.sh scripts/scan-images.sh scripts/smoke-appliance.sh scripts/smoke-firewall-image.sh scripts/smoke-firewall-lxc.sh images/base/first-boot/boetticher-first-boot.sh images/base/runtime/install-runtime-state.sh
	@test -z "$$(rg -n 'BOETTICHER_IMAGE_BUILD_COMMAND|exec sh -c' scripts || true)"

image-base image-dns-blocky image-dns-adguard image-logging image-monitoring image-firewall image-firewall-lxc image-portal image-tailnet-router image-litellm image-printer image-aiops image-gatus images:
	./scripts/build-images.sh $@

scan-base scan-dns-blocky scan-dns-adguard scan-logging scan-monitoring scan-firewall scan-firewall-lxc scan-portal scan-tailnet-router scan-litellm scan-printer scan-aiops scan-gatus scan-images:

...
func runHostTeardown(args []string, input io.Reader, out io.Writer) (err error) {
	...
	display := controllerstatus.StartStandard("host teardown", 4)
	defer func() {
		display.End(err)
	}()
	...
	display.Progress(1, "Internal network removed")
	...
	display.Progress(2, "Dedicated storage removed")
	...
	display.Progress(3, "Host baseline removed")
	...
	display.Progress(4, "Local Host selections cleared")

...
var helpSpecs = map[string]helpSpec{
	...
	"status": {
		Usage: "boetticher status [--site DIR] [--live] [--verbose] [--json]", Purpose: "Show whether the platform and its modules need attention.", Arguments: "No positional arguments.", Options: "--live performs bounded read-only gateway checks; --verbose includes detailed reasons and next actions; --json emits the versioned status model. Exit status is zero only for HEALTHY; FAILED, DEGRADED, and ACTION REQUIRED return non-zero. Disabled optional modules are excluded from the overall result.", Safety: "Read-only. Live transport and malformed observations fail non-zero and are never reported as PASS.", Examples: "boetticher status --site ./my-boetticher; boetticher status --site ./my-boetticher --live --json", Related: "deploy, doctor, verify, dhcp",
	},
	"update": {
	...
	"firewall rule add": {
		Usage: "boetticher firewall rule add [--source SOURCE] [--destination DESTINATION|--vmid VMID] [--protocol PROTOCOL] [--ports PORTS] [--id ID] [--site DIR] [--dry-run] [--confirm] [--json]", Purpose: "Add a bounded user-workload firewall rule to desired site configuration.", Arguments: "--source and --protocol are required; choose exactly one of --destination and --vmid.", Options: "--dry-run previews the change; --confirm writes it; --age-identity, --proxmox-ca, and --insecure apply when resolving a VMID.", Safety: "Changes site.yml only and never deploys. Review the rule before confirming; deployment remains boetticher deploy.", Examples: "boetticher firewall rule add --source TRUSTED --destination 10.10.20.61 --protocol tcp --ports 8080 --confirm --site ./my-boetticher", Related: "firewall diff, deploy, dhcp reservation",
	},
	"firewall rule list": {
	...
	"module configure": {
		Usage: "boetticher module configure MODULE [--site DIR] [--dry-run] [--json] [--non-interactive] [--enabled BOOL] [--set KEY=VALUE] [--secret NAME] [--usb REQUIREMENT=PORT] [--confirm]", Purpose: "Configure one built-in module's desired state.", Arguments: "MODULE is a registered first-party module.", Options: "The interactive workflow asks only for fields the module needs. --set, --secret, and --usb provide repeatable typed inputs; --non-interactive suppresses prompts; --dry-run previews; --confirm applies.", Safety: "Changes local desired state only and never deploys. Secret values come from a prompt or stdin and are never accepted as arguments or shown in plans.", Examples: "boetticher module configure printer --site ./my-boetticher; boetticher module configure aiops --non-interactive --enabled true --set model_alias=operations-investigator --confirm --site ./my-boetticher", Related: "module show, deploy, module secrets",
	},
	"module enable": {
		Usage: "boetticher module enable NAME [--site DIR] [--confirm]", Purpose: "Enable one optional built-in module in desired state.", Arguments: "NAME is a registered optional module.", Options: "--confirm applies the local change; --site selects the private site repository.", Safety: "Changes desired state only and never deploys. Use deploy after reviewing the module's requirements.", Examples: "boetticher module enable monitoring --confirm --site ./my-boetticher", Related: "module configure, deploy",
	},
	"module disable": {
		Usage: "boetticher module disable NAME [--site DIR] [--confirm] [--purge]", Purpose: "Disable one optional built-in module.", Arguments: "NAME is a registered optional module.", Options: "--confirm applies the local change; --purge requests removal of owned resources where supported; --site selects the private site repository.", Safety: "Disable retains owned resources by default. Purge is destructive and requires exact ownership proof and confirmation; neither operation applies the platform until deploy.", Examples: "boetticher module disable printer --confirm --site ./my-boetticher", Related: "module status, deploy, recovery",
	},
	"module status": {
		Usage: "boetticher module status [NAME] [--site DIR] [--live]", Purpose: "Show desired and available status for built-in modules.", Arguments: "NAME optionally limits the view to one registered module.", Options: "--live performs bounded live checks where supported; --site selects the private site repository.", Safety: "Read-only. A local status result does not prove a deployed service journey.", Examples: "boetticher module status printer --site ./my-boetticher", Related: "status, doctor, module list",
	},
	"modules": {

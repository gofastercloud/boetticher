...
var commandSpecs = []commandSpec{
	...
	{Usage: "boetticher foundation status|converge|teardown"},
}
...
var helpSpecs = map[string]helpSpec{
	...
	"foundation": {
		Usage: "boetticher foundation status|converge|teardown", Purpose: "Aggregate, safely converge, or remove the local Controller foundation while preserving trust and HOME management.", Arguments: "status is read-only. converge repeats native checks and stops at trust or destructive approval boundaries. teardown removes exact owned foundation state and requires explicit storage confirmation.", Options: "teardown accepts --plan, --yes, and --confirm-storage /dev/disk/by-id/DEVICE.", Safety: "Teardown never removes unexpected guests, shared packages, unknown host configuration, the boot disk, or Controller↔Proxmox trust.", Examples: "boetticher foundation status; boetticher foundation converge; boetticher foundation teardown --plan; boetticher foundation teardown --confirm-storage /dev/disk/by-id/ata-example --yes", Related: "controller status, host status, storage status, network status",
	},
	"tui": {

...
var advancedCommandSpecs = []commandSpec{
	...
	{Usage: "boetticher deploy [--site DIR] [--age-identity PATH] [--dry-run] [--replace-firewall] [--recreate-legacy-lxcs] [--confirm]"},
...
var helpSpecs = map[string]helpSpec{
	...
	"deploy": {
		Usage: "boetticher deploy [--site DIR] [--age-identity PATH] [--proxmox-ca PATH] [--insecure] [--dry-run] [--rotate-airvpn-profile] [--replace-firewall] [--recreate-legacy-lxcs] [--confirm]", Purpose: "Build or update the Boetticher platform from your saved site settings.", Arguments: "No positional arguments.", Options: "--dry-run shows the plan without changing the lab; --rotate-airvpn-profile makes a new retained AirVPN WireGuard profile; --replace-firewall reimages only the managed firewall root disk after proving its declared persistent volumes; --recreate-legacy-lxcs discards only proven legacy local-raw LXC state and recreates those appliances on planned storage; each recovery action requires --confirm outside a dry-run. Connection options select the Proxmox certificate path. A normal deploy prints nine clear phases and one final PASS or FAIL line.", Safety: "This is the command that changes the platform. Review the plan first. It stops for guests or data volumes it does not recognise, and it keeps AirVPN keys and profiles out of arguments, logs, and public generated files. Firewall recovery never touches its declared persistent volumes. Legacy-LXC recovery validates the complete candidate set before stopping a guest; it never targets QEMU, the firewall, unknown guests, or non-legacy LXC storage. It may accept only the exact retired LiteLLM appliance at VMID 210 as Bifrost's replacement predecessor, and it discards each selected LXC appliance's declared state.", Examples: "boetticher deploy --site ./my-boetticher --dry-run; boetticher deploy --site ./my-boetticher; boetticher deploy --site ./my-boetticher --replace-firewall --confirm; boetticher deploy --site ./my-boetticher --recreate-legacy-lxcs --confirm", Related: "preflight, verify, doctor",
	},
	"status": {

...
var helpSpecs = map[string]helpSpec{
	...
	"module": {
		Usage: "boetticher module list|show|plan|enable|disable|status|secrets [NAME] [--site DIR] [--dry-run] [--confirm] [--purge] [--age-identity PATH] [--proxmox-ca PATH] [--insecure]", Purpose: "Inspect or change first-party module intent and manage declared operator secrets through the shared deploy engine.", Arguments: "NAME is required for show, plan, enable, disable, and secrets, and optional for status.", Options: "--dry-run shows the resolved effect; --confirm authorizes configuration or destructive lifecycle changes and secret removal; --age-identity selects the external Age identity for secret inspection.", Safety: "DNS and logging are mandatory. Ordinary disable retains owned guests and persistent data; purge is destructive. Secret values are read from hidden TTY input or explicit set-command stdin and are never displayed.", Examples: "boetticher module secrets litellm list --site ./my-boetticher", Related: "config validate, deploy, doctor",
	},
	"modules": {
		Usage: "boetticher modules list|MODULE show|plan|enable|disable|status|secrets|purge [--site DIR] [--dry-run] [--confirm] [--age-identity PATH] [--proxmox-ca PATH] [--insecure]", Purpose: "Inspect or change first-party module intent and manage declared operator secrets through the shared registry and deploy engine.", Arguments: "MODULE is a registered first-party module. list retains the generic module inventory; lifecycle and secret commands are resolved by the same generic implementation.", Options: "--dry-run shows the resolved effect; --confirm authorizes configuration, destructive lifecycle changes, and secret removal; --age-identity selects the external Age identity for secret inspection.", Safety: "tailnet-router, litellm, streamdeck, and printer are default-off. Ordinary disable retains owned guests and persistent data; purge is destructive and never treats VMID range membership as ownership. Secret values are never displayed or accepted as command arguments.", Examples: "boetticher modules list --site ./my-boetticher; boetticher modules litellm secrets set openrouter_api_key --site ./my-boetticher", Related: "config validate, deploy, doctor",
	},
	"config": {

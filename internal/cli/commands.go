...
var helpSpecs = map[string]helpSpec{
	...
	"module firewall": {
		Usage: "boetticher module firewall plan|apply|status|teardown [--site DIR] [--age-identity PATH] [--yes]", Purpose: "Plan, apply, inspect, or remove the first-party firewall capability.", Arguments: "The firewall capability manages one bounded provider appliance and six LAB IPv4 gateways.", Options: "--site selects the private site; --age-identity selects the routine Age identity; --yes approves the initial apply or destructive teardown.", Safety: "Plan and status are read-only. Apply consumes the Host-owned vmbr1 substrate and is idempotent. Teardown removes only the firewall provider and its Controller-local provider state; it preserves Host trust, storage, vmbr0, vmbr1, and physical networking.", Examples: "boetticher module firewall plan; boetticher module firewall apply --yes; boetticher module firewall status; boetticher module firewall teardown --yes", Related: "host apply, host status, module",
	},
	"host enroll": {

...
var commandSpecs = []commandSpec{
	...
	{Usage: "boetticher logs [HOST] [--unit UNIT] [--since DURATION] [--priority LEVEL] [--limit N]"},
...
var helpSpecs = map[string]helpSpec{
	...
	"logs": {
		Usage: "boetticher logs [HOST] [--unit UNIT] [--since DURATION] [--priority LEVEL] [--limit N]", Purpose: "Read a bounded journal view through the central collector using the normal Proxmox bastion path.", Arguments: "HOST is a known boetticher-managed or retained endpoint; omitted HOST reads the collector-local journal.", Options: "--unit accepts a systemd service unit; --since accepts a positive duration up to 168h; --priority accepts a fixed journal priority; --limit is 1-500 and defaults to 100.", Safety: "Read-only. Output is bounded; there is no follow mode, TUI, arbitrary journal path, or query language. Central logging is asynchronous and not an availability dependency.", Examples: "boetticher logs lab-dns-01 --since 1h; boetticher logs lab-fw-01 --priority warning --limit 100", Related: "doctor, verify",
	},
	"verify": {

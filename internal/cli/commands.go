...
var advancedCommandSpecs = []commandSpec{
	...
	{Usage: "boetticher modules list|MODULE show|plan|configure|enable|disable|status|purge [--site DIR] [--dry-run] [--json] [--confirm] [--age-identity PATH] [--proxmox-ca PATH] [--insecure]"},
...
var helpSpecs = map[string]helpSpec{
	...
	"storage": {
		Usage: "boetticher storage status|initialize [--site DIR] [--live] [--storage-confirmed]", Purpose: "Inspect the Core-owned storage substrate or perform its explicitly confirmed initialization.", Arguments: "status inspects; initialize prepares the configured dedicated data profile.", Options: "--live reads the Proxmox host; --storage-confirmed authorizes fixed-device initialization.", Safety: "initialize can format the explicitly configured device. Modules cannot select disks or create VGs.", Examples: "boetticher storage status --site ./my-boetticher", Related: "bootstrap, deploy, doctor",
	},
	"module": {
...
var nestedHelpSpecs = map[string]helpSpec{
	...
	"module configure":         helpSpecs["module"],

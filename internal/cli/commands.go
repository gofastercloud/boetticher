...
var helpSpecs = map[string]helpSpec{
	...
	"init": {
		Usage: "boetticher init [--site-dir DIR] [--age-identity PATH] [--external-firewall]", Purpose: "Create a site directory and the recovery material Boetticher needs.", Arguments: "No positional arguments.", Options: "--site-dir selects the site directory; --age-identity selects your private age identity; --external-firewall says that you run the gateway yourself.", Safety: "Creates local files only. It does not contact or change Proxmox.", Examples: "boetticher init --site-dir ./my-boetticher", Related: "config validate, preflight, bootstrap",
	},
	"preflight": {
	...
	"bootstrap": {
		Usage: "boetticher bootstrap [--site DIR] [--age-identity PATH] [--recovery-confirmed] [--storage-confirmed] [--operator-key PATH] [--initial-user USER] [--known-hosts PATH] [--proxmox-ca PATH] [--insecure] [--trunk-interface IFACE] [--dry-run] [--cleanup]", Purpose: "Prepare the Proxmox connection, bridges, storage, temporary builder, and appliance images.", Arguments: "No positional arguments.", Options: "--dry-run renders only; --cleanup removes Boetticher's stale temporary builder without rebuilding images; --recovery-confirmed confirms your independent age copy; --storage-confirmed approves the selected data disk; --operator-key and --initial-user select the first SSH login; --known-hosts selects the host-key file; --proxmox-ca selects the Proxmox API CA file; --insecure explicitly allows self-signed API TLS; --trunk-interface selects the physical trunk.", Safety: "This can change Proxmox setup and creates a temporary builder. It only removes VMID 190 when its expected name and tag match. Compare the first SSH fingerprint before accepting it.", Examples: "boetticher bootstrap --site ./my-boetticher --recovery-confirmed --proxmox-ca /path/to/pve-root-ca.pem; boetticher bootstrap --site ./my-boetticher --cleanup --age-identity /path/to/age-identity.txt --proxmox-ca /path/to/pve-root-ca.pem", Related: "preflight, deploy, verify",
	},
	"deploy": {
		Usage: "boetticher deploy [--site DIR] [--age-identity PATH] [--proxmox-ca PATH] [--insecure] [--dry-run] [--rotate-airvpn-profile] [--confirm]", Purpose: "Build or update the Boetticher platform from your saved site settings.", Arguments: "No positional arguments.", Options: "--dry-run shows the plan without changing the lab; --rotate-airvpn-profile makes a new retained AirVPN WireGuard profile; --confirm approves an operation that can replace a Boetticher appliance; connection options select the Proxmox certificate path. A normal deploy prints nine clear phases and one final PASS or FAIL line.", Safety: "This is the command that changes the platform. Review the plan first. It stops for guests or data volumes it does not recognise, and it keeps AirVPN keys and profiles out of arguments, logs, and public generated files.", Examples: "boetticher deploy --site ./my-boetticher --dry-run; boetticher deploy --site ./my-boetticher; boetticher deploy --site ./my-boetticher --rotate-airvpn-profile", Related: "preflight, verify, doctor",
	},
	"status": {
	...
	"storage": {
		Usage: "boetticher storage status|initialize [--site DIR] [--live] [--storage-confirmed] [--initial-user USER] [--known-hosts PATH]", Purpose: "Inspect storage or prepare the dedicated data-disk profile you selected.", Arguments: "status inspects; initialize prepares the configured dedicated-data profile.", Options: "--live reads the Proxmox host; --storage-confirmed approves fixed-device setup; --initial-user and --known-hosts select the SSH route.", Safety: "initialize can format the configured device. Modules cannot choose disks or create volume groups.", Examples: "boetticher storage status --site ./my-boetticher", Related: "bootstrap, deploy, doctor",
	},
	"module": {
		Usage: "boetticher module list|show|plan|configure|enable|disable|status [NAME] [--site DIR] [--dry-run] [--json] [--confirm] [--purge] [--age-identity PATH] [--proxmox-ca PATH] [--insecure]", Purpose: "Browse, configure, and run Boetticher's built-in modules.", Arguments: "NAME is needed for show, plan, configure, enable, and disable; status can take one too.", Options: "--dry-run shows the result; --json emits a secret-free setup plan; --confirm approves configuration or lifecycle changes; configure accepts repeatable --set KEY=VALUE, --usb REQUIREMENT=PORT, and --secret NAME (value from stdin).", Safety: "Configure updates site.yml and encrypted secrets only; it never deploys. Confirmed enable and disable run deploy. DNS and logging stay on. Disable keeps a module's guests and data unless you explicitly add --purge.", Examples: "boetticher module configure printer --site ./my-boetticher; boetticher module configure aiops --enabled true --set model_alias=operations-investigator --confirm --site ./my-boetticher", Related: "config validate, deploy, doctor",
	},
	"module secrets": {
	...
	"module enable": {
		Usage: "boetticher module enable NAME [--site DIR] [--dry-run] [--confirm] [--age-identity PATH] [--proxmox-ca PATH] [--insecure]", Purpose: "Turn on one optional module and deploy it.", Arguments: "NAME is a registered optional module.", Options: "--dry-run shows the plan without changing the site; --confirm saves the setting and runs deploy; connection options select the certificate path.", Safety: "A confirmed enable changes saved settings and immediately deploys. Review the plan first; --dry-run does neither.", Examples: "boetticher module enable monitoring --confirm --site ./my-boetticher", Related: "module configure, deploy",
	},
	"module disable": {
		Usage: "boetticher module disable NAME [--site DIR] [--dry-run] [--confirm] [--purge] [--age-identity PATH] [--proxmox-ca PATH] [--insecure]", Purpose: "Turn off one optional module and deploy the change.", Arguments: "NAME is a registered optional module.", Options: "--dry-run shows the plan without changing the site; --confirm saves the setting and runs deploy; --purge asks to remove the module's resources; connection options select the certificate path.", Safety: "Disable keeps a module's guests and data by default. --purge permanently removes only recognised module resources and always needs confirmation.", Examples: "boetticher module disable printer --confirm --site ./my-boetticher", Related: "module status, deploy, recovery",
	},
	"module status": {
	...
	"modules": {
		Usage: "boetticher modules list|MODULE show|plan|configure|enable|disable|status|secrets|purge [--site DIR] [--dry-run] [--json] [--confirm] [--age-identity PATH] [--proxmox-ca PATH] [--insecure]", Purpose: "Use module commands in the alternate, module-first form.", Arguments: "MODULE is a registered module. list shows the menu; module-specific commands use the same implementation as the singular form.", Options: "--dry-run shows the change; --confirm approves configuration, removal, or secret deletion; configure accepts repeatable --set KEY=VALUE, --usb REQUIREMENT=PORT, and --secret NAME (value from stdin).", Safety: "tailnet-router, airvpn, bifrost, printer, and streamdeck start off. AirVPN profile rotation is always explicit. Configure saves settings; confirmed enable and disable deploy. Secret values are never displayed or command arguments.", Examples: "boetticher modules printer configure --site ./my-boetticher; boetticher modules aiops configure --set model_alias=operations-investigator --site ./my-boetticher", Related: "config validate, deploy, doctor",
	},
	"config": {
...
var nestedHelpSpecs = map[string]helpSpec{
	...
	"network test":             helpSpec{Usage: "boetticher network test [--site DIR] [--zones ZONE,...] [--capture] [--cleanup-only] [--json] [--age-identity PATH] [--proxmox-ca PATH] [--insecure]", Purpose: "Check routes, DNS, policy, mTLS, and speed from temporary probes in selected zones.", Arguments: "No positional arguments. By default it visits all six modeled zones.", Options: "--zones selects a comma-separated subset; --capture adds a short tcpdump from a probe; --cleanup-only removes a stale Boetticher probe; --json is for tools; connection options select the Proxmox certificate path.", Safety: "Advanced and live. It creates only recognised unprivileged LXC probes in VMIDs 910-919 and never changes firewall policy. It tries cleanup after every run; resolve a cleanup failure before retrying. It leaves your saved settings alone.", Examples: "boetticher network test --site ./my-boetticher; boetticher network test --zones TRUSTED,SANDBOX --json --site ./my-boetticher", Related: "network trunk status, firewall diff, dhcp leases, verify"},
...
func CommandReferenceMarkdown() string {
	...
	document.WriteString("This page is generated from the same usage menu as `boetticher help`. Most days you will use `deploy`, `status`, `doctor`, and `module configure`. Add `--help` to any command for the friendly, full explanation.\n\n")

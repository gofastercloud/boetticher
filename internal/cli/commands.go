...
var advancedCommandSpecs = []commandSpec{
	...
	{Usage: "boetticher storage status|initialize [--site DIR] [--live] [--storage-confirmed] [--reinitialize] [--initial-user USER] [--known-hosts PATH]"},
...
var helpSpecs = map[string]helpSpec{
	...
	"storage": {
		Usage: "boetticher storage status|initialize [--site DIR] [--live] [--storage-confirmed] [--reinitialize] [--initial-user USER] [--known-hosts PATH]", Purpose: "Inspect storage or prepare the dedicated data-disk profile you selected.", Arguments: "status inspects; initialize prepares the configured dedicated-data profile.", Options: "--live reads the Proxmox host; --storage-confirmed approves fixed-device setup; --reinitialize permits replacement of an old unmounted, non-LVM layout on the exact configured device; --initial-user and --known-hosts select the SSH route.", Safety: "initialize can format the configured device. It refuses the system disk, mounts, active swap, and LVM use; --reinitialize is separate from ordinary confirmation. Modules cannot choose disks or create volume groups.", Examples: "boetticher storage initialize --site ./my-boetticher --storage-confirmed --reinitialize --known-hosts ~/.ssh/known_hosts", Related: "bootstrap, deploy, doctor",
	},
	"module": {

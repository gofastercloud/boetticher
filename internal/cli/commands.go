...
var commandSpecs = []commandSpec{
	...
	{Usage: "boetticher enroll [--site DIR] [--bootstrap-address ADDRESS] [--age-identity PATH] [--recovery-confirmed] [--storage-confirmed]"},
...
var helpSpecs = map[string]helpSpec{
	...
	"enroll": {
		Usage: "boetticher enroll [--site DIR] [--bootstrap-address ADDRESS] [--age-identity PATH] [--recovery-confirmed] [--storage-confirmed]", Purpose: "Enroll the controller with the Proxmox host and prepare authenticated scoped access.", Arguments: "No positional arguments.", Options: "--bootstrap-address records the fresh Proxmox HOME-side address on first enrollment; recovery and storage confirmations approve the independent recovery copy and selected data disk.", Safety: "This is the bounded setup operation. It does not build appliance images; normal deployment consumes a separately signed release bundle.", Examples: "boetticher enroll --site ./my-boetticher --bootstrap-address 192.0.2.10 --recovery-confirmed", Related: "init, bundle import, plan",
	},
	"plan": {

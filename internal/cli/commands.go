...
var helpSpecs = map[string]helpSpec{
	...
	"modules": {
		Usage: "boetticher modules list|MODULE show|plan|configure|enable|disable|status|secrets|purge [--site DIR] [--dry-run] [--json] [--confirm] [--age-identity PATH]", Purpose: "Use module commands in the alternate, module-first form.", Arguments: "MODULE is a registered module. list shows the menu; module-specific commands use the same implementation as the singular form.", Options: "--dry-run shows the change; --confirm approves configuration, removal, or secret deletion; configure accepts repeatable --set KEY=VALUE, --usb REQUIREMENT=PORT, and --secret NAME (value from stdin).", Safety: "tailnet-router, airvpn, bifrost, printer, and streamdeck start off. AirVPN profile rotation is always explicit. Configuration changes desired state only; deploy remains explicit. Secret values are never displayed or command arguments.", Examples: "boetticher modules printer configure --site ./my-boetticher; boetticher modules aiops configure --set model_alias=operations-investigator --site ./my-boetticher", Related: "config validate, plan, deploy",
	},
	"config": {

...
var commandSpecs = []commandSpec{
	...
	{Usage: "boetticher deploy [--site DIR] [--age-identity PATH] [--proxmox-ca PATH] [--insecure] [--dry-run] [--rotate-airvpn-profile] [--replace-firewall] [--confirm]"},
...
var helpSpecs = map[string]helpSpec{
	...
	"deploy": {
		Usage: "boetticher deploy [--site DIR] [--age-identity PATH] [--proxmox-ca PATH] [--insecure] [--dry-run] [--rotate-airvpn-profile] [--replace-firewall] [--confirm]", Purpose: "Build or update the Boetticher platform from your saved site settings.", Arguments: "No positional arguments.", Options: "--dry-run shows the plan without changing the lab; --rotate-airvpn-profile makes a new retained AirVPN WireGuard profile; --replace-firewall reimages only the managed firewall root disk after proving its declared persistent volumes and requires --confirm outside a dry-run; --confirm approves an operation that can replace a Boetticher appliance; connection options select the Proxmox certificate path. A normal deploy prints nine clear phases and one final PASS or FAIL line.", Safety: "This is the command that changes the platform. Review the plan first. It stops for guests or data volumes it does not recognise, and it keeps AirVPN keys and profiles out of arguments, logs, and public generated files. Firewall recovery never touches its declared persistent volumes.", Examples: "boetticher deploy --site ./my-boetticher --dry-run; boetticher deploy --site ./my-boetticher; boetticher deploy --site ./my-boetticher --replace-firewall --confirm", Related: "preflight, verify, doctor",
	},
	"status": {

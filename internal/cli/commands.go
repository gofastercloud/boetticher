...
var helpSpecs = map[string]helpSpec{
	...
	"aiops": {
		Usage: "boetticher aiops status [--site DIR] [--live] [--json]", Purpose: "Inspect desired or bounded live AIOps incident lifecycle and usage state.", Arguments: "status is the only operation.", Options: "--live reads the adapter database through the normal bastion; --json emits machine-readable output.", Safety: "Read-only. No investigation, note, acknowledgement, clearing, restart, or remediation is triggered.", Examples: "boetticher aiops status --site ./my-boetticher --live", Related: "module status, doctor, logs",
	},
	"verify": {
	...
	"modules": {
		Usage: "boetticher modules list|MODULE show|plan|enable|disable|status|secrets|purge [--site DIR] [--dry-run] [--confirm] [--age-identity PATH] [--proxmox-ca PATH] [--insecure]", Purpose: "Inspect or change first-party module intent and manage declared operator secrets through the shared registry and deploy engine.", Arguments: "MODULE is a registered first-party module. list retains the generic module inventory; lifecycle and secret commands are resolved by the same generic implementation.", Options: "--dry-run shows the resolved effect; --confirm authorizes configuration, destructive lifecycle changes, and secret removal; --age-identity selects the external Age identity for secret inspection.", Safety: "tailnet-router, litellm, and printer are default-off. Ordinary disable retains owned guests and persistent data; purge is destructive and never treats VMID range membership as ownership. Secret values are never displayed or accepted as command arguments.", Examples: "boetticher modules list --site ./my-boetticher; boetticher modules printer plan --site ./my-boetticher", Related: "config validate, deploy, doctor",
	},
	"config": {

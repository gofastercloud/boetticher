...
var helpSpecs = map[string]helpSpec{
	...
	"module secrets": {
		Usage: "boetticher module secrets MODULE list|set|remove|rotate [--site DIR] [--age-identity PATH] [--confirm]", Purpose: "Inspect and manage declared operator-supplied secrets for a first-party module.", Arguments: "MODULE is a registered first-party module; list, set, and remove select the secret operation, with NAME required for set and remove. AirVPN rotate removes the Core-generated retained profile and requires no NAME.", Options: "--site selects the private site repository; --age-identity selects the operator-owned private Age identity; --confirm is required for secret removal and AirVPN profile rotation.", Safety: "Secret values are read from a hidden prompt or stdin and are never accepted as arguments, displayed, logged, or written to generated output. The command changes encrypted desired state only; it never deploys. AirVPN rotation is limited to the Core-generated profile.", Examples: "boetticher module secrets litellm list --site ./my-boetticher; boetticher module secrets litellm set openrouter_api_key --site ./my-boetticher; boetticher module secrets litellm remove openrouter_api_key --confirm --site ./my-boetticher; boetticher module secrets airvpn rotate --confirm --site ./my-boetticher", Related: "module configure, deploy, config validate",
	},
	"module list": {

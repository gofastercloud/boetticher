...
func TestZabbixPasswordUsesFixedPostgresTransition(t *testing.T) {
	runner := &credentialDeploymentRunner{}
	if err := installZabbixAPIPassword(context.Background(), runner, "10.10.10.20", "synthetic-zabbix-secret"); err != nil {
		t.Fatal(err)
	}
	if len(runner.values) != 1 || !strings.Contains(runner.values[0], "synthetic-zabbix-secret") {
		t.Fatal("Zabbix password was not streamed through protected stdin")
	}
	if runner.users[0] != "root" || runner.commands[0] != "runuser -u postgres -- psql --dbname zabbix --set ON_ERROR_STOP=1" {
		t.Fatalf("Zabbix password transition was not fixed and root-transported: users=%v commands=%v", runner.users, runner.commands)
	}
}

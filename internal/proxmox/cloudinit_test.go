...
func TestFirewallCloudInitUsesStableInterfaceIdentities(t *testing.T) {
	...
	for _, value := range []string{"set-name: wan0", "set-name: trusted0", "set-name: servers0", "set-name: sandbox0", "set-name: mgmt0", "set-name: transit0", "set-name: infra0", "net.ipv4.ip_forward=0", "net.ipv6.conf.all.forwarding=0", "/bin/cat /var/lib/kea/kea-leases4.csv"} {

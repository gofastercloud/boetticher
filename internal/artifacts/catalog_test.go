...
func TestBuildSourceArchiveIsAllowListedAndDeterministic(t *testing.T) {
	...
	for _, required := range []string{"buildbundle.go", "scripts/build-images.sh", "images/base/debian.yaml", "images/tailnet-router/image.yaml", "images/litellm/runtime/requirements.lock", "cmd/qualify-artifact/main.go", "cmd/boetticher-aiops/main.go"} {
		if !entries[required] {

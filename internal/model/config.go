...
func (m ModulesConfig) Map() map[string]ModuleConfig {
	...
	if m.DNS != nil {
		result["dns"] = ModuleConfig{Enabled: cloneBool(m.DNS.Enabled), Provider: string(m.DNS.Provider)}
	}
	if m.Monitoring != nil {
...
func ModulesConfigFromMap(input map[string]ModuleConfig) ModulesConfig {
	...
	if config, ok := input["dns"]; ok {
		result.DNS = &DNSModuleConfig{Enabled: cloneBool(config.Enabled), Provider: DNSProvider(config.Provider)}
	}
...
func (m *ModulesConfig) Set(name string, config ModuleConfig) error {
	switch name {
	case "dns":
		m.DNS = &DNSModuleConfig{Enabled: cloneBool(config.Enabled), Provider: DNSProvider(config.Provider)}
	case "monitoring":

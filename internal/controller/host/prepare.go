...
func CheckBaseline(ctx context.Context, transport Transport) (bool, error) {
	result, err := transport.Run(ctx, "test -f /etc/apt/sources.list.d/boetticher-pve-no-subscription.sources && test -f /etc/systemd/logind.conf.d/90-boetticher-headless.conf && dpkg-query -W -f='${Status}' rsync 2>/dev/null | grep -qx 'install ok installed' && dpkg-query -W -f='${Status}' make 2>/dev/null | grep -qx 'install ok installed' && test -x /usr/local/libexec/boetticher-host-speedtest")
	if err != nil {
		return false, nil
	}

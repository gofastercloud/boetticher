...
func (c HostClient) ReconcileCollection(ctx context.Context, b Binding, payloadRoot, publicDomain string, config CollectionConfig) error {
	...
	configCommand := fmt.Sprintf("pct exec %d -- sh -c %s", b.VMID, shellQuoteValue("set -eu; install -d -m 0750 /etc/boetticher/observability; tmp=$(mktemp /etc/boetticher/observability/.collection.XXXXXX); trap 'rm -f -- \"$tmp\"' EXIT HUP INT TERM; cat >\"$tmp\"; chmod 0640 \"$tmp\"; mv -f \"$tmp\" "+CollectionConfigPath))

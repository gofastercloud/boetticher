...
func (c HostClient) AskHolmes(ctx context.Context, b Binding, alias string, question io.Reader) (string, error) {
	...
	script := fmt.Sprintf("set -eu; cred_dir=$(mktemp -d /run/boetticher-holmes-ask.XXXXXX); trap 'rm -rf -- \"$cred_dir\"' EXIT HUP INT TERM; chown holmes:holmes \"$cred_dir\"; chmod 0700 \"$cred_dir\"; install -o holmes -g holmes -m 0400 /var/lib/boetticher/credentials/holmes-client-token.cred \"$cred_dir/holmes-client-token\"; timeout --signal=TERM --kill-after=5s %ds setpriv --reuid=holmes --regid=holmes --init-groups --no-new-privs env -i HOME=/var/lib/boetticher/holmes PATH=/opt/boetticher/observability/holmes/venv/bin:/usr/bin:/bin PYTHONNOUSERSITE=1 HOLMES_CONFIGPATH_DIR=/etc/boetticher/holmes CREDENTIALS_DIRECTORY=\"$cred_dir\" /opt/boetticher/observability/holmes/venv/bin/python /opt/boetticher/observability/holmes/holmes-runner.py --model-alias %s", holmesUnitTimeoutSeconds, shellQuoteValue(alias))

...
func TestLoadSettingsUsesSmallDefaultsAndReferenceThreshold(t *testing.T) {
	...
	if settings.Interval != 30*time.Second || settings.PingInterval != time.Minute || settings.ThroughputInterval != time.Hour || settings.HealthyMbps != 500 {
...
func TestLoadSettingsReadsOptionalStatusValues(t *testing.T) {
	...
	if settings.Interval != 45*time.Second || settings.ThroughputInterval != 15*time.Minute || settings.HealthyMbps != 250 || !settings.BlinktEnabled || settings.GPIOChip != 2 || settings.Brightness != 0.2 {

...
func TestParseStreamDeckUSBDescriptorFindsHIDEndpoints(t *testing.T) {
	descriptor := []byte{
		18, 1, 0, 2, 0, 0, 0, 64, 0xd9, 0x0f, 0x6d, 0x00, 0, 0, 0, 0, 0, 1,
		9, 2, 32, 0, 1, 1, 0, 0, 0,
		9, 4, 0, 0, 2, 3, 0, 0, 0,
...
func TestParseStreamDeckUSBDescriptorRejectsUnknownIdentity(t *testing.T) {
	descriptor := []byte{
		18, 1, 0, 2, 0, 0, 0, 64, 0xff, 0x0f, 0x6d, 0x00, 0, 0, 0, 0, 0, 1,
	}
	if _, err := parseStreamDeckUSBDescriptor(descriptor); err == nil {
		t.Fatal("unknown USB identity was accepted")
	}
}

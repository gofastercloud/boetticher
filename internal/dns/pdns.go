...
func PrimaryCommandPlan(plan Plan) []PowerDNSCommand {
	...
	for _, record := range plan.StaticRecords {
		commands = append(commands, PowerDNSCommand{Args: []string{"pdnsutil", "replace-rrset", plan.StaticZone, zoneRelativeName(record.Name, plan.StaticZone), record.Type, "300", record.Address}})
	}
	return commands
}

...
func StatusCommand(device string) (string, error) {
	...
	return strings.Join([]string{
		...
		"device=" + quoted,
		"printf 'device=%s\\n' \"$device\"",
		"printf 'device_path=%s\\n' \"$(readlink -f \"$device\" 2>/dev/null || printf missing)\"",
		"printf 'volume_group=%s\\n' \"$(vgs --noheadings --select vg_name=" + VolumeGroup + " -o vg_name 2>/dev/null | xargs || printf missing)\"",
		"printf 'thin_pool=%s\\n' \"$(lvs --noheadings --select lv_name=" + ThinPool + " -o lv_name " + VolumeGroup + " 2>/dev/null | xargs || printf missing)\"",
		"printf 'backup_lv=%s\\n' \"$(lvs --noheadings --select lv_name=" + BackupLogicalVol + " -o lv_name " + VolumeGroup + " 2>/dev/null | xargs || printf missing)\"",
		"printf 'filesystem=%s\\n' \"$(blkid -s TYPE -o value /dev/" + VolumeGroup + "/" + BackupLogicalVol + " 2>/dev/null || printf missing)\"",
		"printf 'mount=%s\\n' \"$(findmnt -no TARGET /dev/" + VolumeGroup + "/" + BackupLogicalVol + " 2>/dev/null || printf missing)\"",
		...
		"printf 'capacity=%s\\n' \"$(df -hP " + BackupMount + " 2>/dev/null | tail -n 1 || printf unavailable)\"",
	}, "\n"), nil
}

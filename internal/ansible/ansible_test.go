...
func TestRunUsesAnsibleStdinPathForExtraVars(t *testing.T) {
	...
	script := "#!/bin/sh\nprintf '%s\\n' \"$@\" > \"$ANSIBLE_ARGS_FILE\"\nprintf '%s' \"$ANSIBLE_FORKS\" > \"$ANSIBLE_FORKS_FILE\"\ncat > \"$ANSIBLE_INPUT_FILE\"\nif [ -n \"$BOETTICHER_ANSIBLE_TIMING_FILE\" ]; then printf '%s\\n' '{\"host\":\"lab-fw-01\",\"task\":\"fake task\",\"path\":\"fake.yml:1\",\"status\":\"ok\",\"duration_ms\":3,\"changed\":false,\"markers\":[\"dns-metadata-update:servers.lab.home.arpa:ALLOW-DNSUPDATE-FROM\"]}' '{\"event\":\"task_batch\",\"task\":\"fake batch\",\"path\":\"fake.yml:2\",\"duration_ms\":7}' >> \"$BOETTICHER_ANSIBLE_TIMING_FILE\"; fi\n"
	...
	if len(result.TaskTimings[0].Markers) != 1 || result.TaskTimings[0].Markers[0] != "dns-metadata-update:servers.lab.home.arpa:ALLOW-DNSUPDATE-FROM" {

...
The pinned `bpg/proxmox` provider configuration manages only VMIDs 100, 110, 111, 120, and 130: the firewall VM and Debian LXC foundation guests. The HCL also filters defensively to that explicit V1 allow-list; it does not discover or import arbitrary existing guests. The provider token is supplied at runtime through a generated, short-lived, or SOPS-decoded variable; it is never committed. OpenTofu state, plans, plugin caches, and lock material are runtime state and stay outside the private site repository.

The generated guest tags are model-owned metadata. They identify the guests as
`boetticher`, `managed`, and `platform`, with role tags and `backup` where the
guest belongs in the platform backup projection. User guests are not selected
or tagged by this stack.

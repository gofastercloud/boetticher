...
These rules have priority over later efficiency or delegation guidance. Preserve
security, correctness, user authority, and required verification gates.

- Optimize total consumption across all agents. Accept slower execution when it
  reduces consumption without reducing correctness or required gates.
- Select the lowest suitable model and effort: use `gpt-5.6-luna` for routine
  research, coding, testing, and browser work; use `gpt-5.6-terra` for deeper
  review only when justified; never default to High effort. Astra is for
  orchestration only. Consider a manually verified Sol handover for a settled,
  bounded backlog, and never run Astra and Sol together.
- Give agents self-contained, small assignments with `fork_turns: none` and
  only the relevant scope. Reuse an agent for related work through acceptance;
  start fresh for unrelated packages and do not let agents spawn children.
- Use sequential execution by default. Use concurrency only when it reduces
  total work or rework, or an explicit deadline requires it.
- Reuse verified evidence and refresh only when current state or required gates
  demand it. Perform one independent consequential review; do not duplicate
  checks unless there are changes, failures, concerns, or fresh required
  evidence.
- Keep searches narrow and results bounded. Read applicable authority once per
  workstream; inspect only relevant changes; read memory and skills once when
  applicable; batch independent operations; refresh evidence when current state
  or required gates demand it; stop checks when the relevant gates pass.
- Do not use frequent polling, timers for unchanged checks, or administrative
...
- Run efficiency checks at meaningful boundaries and record corrective actions
  ...
  output tokens accurately; raw totals are not charges or fixed savings.
- Act as coordinator and product owner: own scope, priorities, integration,
  verification, and the final outcome. Keep one owner for live systems and Git
  delivery; delegation grants no new authority. Give each delegate the outcome,
  exact read/write scope, constraints, acceptance criteria, and return format.
  Review delegate changes and report changed paths, checks, findings, and
  blockers rather than transcripts. Preserve unrelated work and never restore
  files to HEAD to undo an agent mistake; revert only your own exact edit and
  report uncertain overlap.
...
- Keep hardware/addresses in reference bindings. Pass Host configuration
  ...
  Roadmap: 4A firewall, 4B client services, 4C VPN, 4D Tailnet, 4E integration
  and recovery, Stage 5 observability, Stage 6 applications. Future plans do not

...
- **Physical NIC renamed or replaced:** use `homelab preflight --live` to compare stable MAC/PCI identity, review the proposed binding, and use the explicit trunk workflow. Do not silently rewrite a renamed or ambiguous interface.
- **Failed trunk mutation:** retain the HOME-side path, inspect `homelab network trunk status --live`, and treat an uncertain rollback as `HOLD` until the Proxmox network state and bastion path are reverified.

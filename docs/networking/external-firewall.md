...
On an OPNsense appliance, create VLAN interfaces 5, 10, 20, 50, and 99 on the
physical trunk, assign the five `.1` addresses, and create DHCP scopes for the
four client VLANs matching the table above; TRANSIT does not receive a DHCP
scope. Put the generated policy into the firewall rules for each interface and
configure RFC2136/TSIG under the DHCP/DDNS features if your
...
five gateway addresses, and attach one DHCP server/scope to each client VLAN.
TRANSIT does not receive a DHCP scope. Place
...
Define the five bridge VLANs on the trunk port, create one logical interface per
zone with its `.1` address, and attach DHCP scopes to the four client VLANs.
TRANSIT does not receive a DHCP scope. Put the generated policy in

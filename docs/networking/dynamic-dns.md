...
Dynamic child zones are fixed to `trusted.lab.home.arpa` and
`sandbox.lab.home.arpa`, with the corresponding reverse zones. TRANSIT, INFRA,
SERVERS, and MGMT are static-only and do not originate DHCP or DDNS updates.
Platform names remain in the static `lab.home.arpa` zone and cannot be claimed
by DHCP clients. Invalid names are ignored, conflicts follow the deterministic
reject-new-active-lease policy, and DDNS failure does not deny a lease.

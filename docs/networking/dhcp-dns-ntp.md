...
SERVERS receives reservation-only DHCP and authenticated DDNS updates; its
reservations are explicit MAC-to-address mappings for user workloads. TRUSTED
uses `10.10.10.10` and `10.10.10.11` for DNS and NTP; SERVERS uses the same
...
do not depend on DHCP for their own operation. SERVERS reservations are for
user-workload addresses and do not transfer workload ownership to boetticher.

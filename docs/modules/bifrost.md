...
`bifrost` is an optional, default-off first-party AI API router at
...
`api_key_secret`. Add secret values with `boetticher module secrets bifrost`
...
the OpenAI-compatible Bifrost contract currently required by AIOps, including
...
The `boetticher-bifrost` artifact contains the Bifrost Go router and nginx; it
does not install Python, a virtual environment, or the former Bifrost package

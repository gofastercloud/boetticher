...
Lab-in-a-Box automatically manages monitoring for its own platform hosts and services only. User workloads are not adopted because they are visible to Proxmox or Zabbix.
...
Use the zone policy to permit only the required monitoring path. Keep user objects outside the generated `Lab-in-a-Box/Platform` host group and preserve their own names/tags. Lab-in-a-Box convergence must not delete or overwrite them.

...
Use the zone policy to permit only the required monitoring path. Keep user objects outside the generated `boetticher/platform` host group and preserve their own names/tags. boetticher deploy must not delete or overwrite them.

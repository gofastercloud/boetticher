...
V1 does not invent arbitrary address allocation or remote-access behavior. A future official platform capability may use `homelab module enable|disable`, but that extension point is not a generic application lifecycle API. A workload that needs new routing, a new provider, or a new security boundary remains a separate module design or user-owned integration.

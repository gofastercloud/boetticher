...
The platform has one canonical model: the normalized `site.yml` plus the enabled module declarations and relevant public secret metadata. For a fixed Lab-in-a-Box version, site, enabled modules, and generated secret identities/metadata, the desired-state model and its projections are deterministic.
...
The repository must not contain the Age private identity. Keep it under the operator-owned restrictive path `~/.config/labinabox/age/identity.txt` or an explicitly selected equivalent.
...
`homelab doctor` checks projections as `ABSENT`, `CURRENT`, or `INCONSISTENT` against the current revision. A current projection proves only model consistency, not deployment or authenticated service behavior.
